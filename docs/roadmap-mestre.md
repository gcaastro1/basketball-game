...
| **1.5 Consolidação** | Arquitetura N jogadores, bola com voo real, aro físico, placar por time/zona, reinício de posse, fim de partida, layers, CI | Loop cesta→placar→reinício testado | ✅ código / ⏳ validação no Unity |
| 2 Fundamentos | Arremesso por distância/contest/timing, pulo, block, steal, rebote/box-out, bandeja/enterrada básicas, Input Actions + buffer | Cena de teste por fundamento + testes | ⏳ próxima |
